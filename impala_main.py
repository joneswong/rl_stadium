...
nest = tf.contrib.framework.nest# this object can perform operations on nested structures (assume no recurisve reference)
...
class Agent(snt.RNNCore):# as Sonnet module targeting for easy tf variable sharing, __call__() calls an internal func wrapped in a Template, which in turn wraps the `_build()` method
  """Agent with ResNet."""
  ...
  def __init__(self, num_actions):
    ...
    with self._enter_variable_scope():# returns a contextlib.contextmanager to enter the internal variable scope
      self._core = tf.contrib.rnn.LSTMBlockCell(256)# has state_size and output_size properties

  def initial_state(self, batch_size):
    return self._core.zero_state(batch_size, tf.float32)# zero_state(batch_size, dtype) returns zero-filled state tensor(s)

  def _instruction(self, instruction):
    # Split string.
    splitted = tf.string_split(instruction)# (source, dlimiter=' ', skip_empty=True) returns a sparse tensor
    dense = tf.sparse_tensor_to_dense(splitted, default_value='')
    length = tf.reduce_sum(tf.to_int32(tf.not_equal(dense, '')), axis=1)# shape (None)

    # To int64 hash buckets. Small risk of having collisions. Alternatively, a
    # vocabulary can be used.
    num_hash_buckets = 1000
    buckets = tf.string_to_hash_bucket_fast(dense, num_hash_buckets)# Converts each string in the input Tensor to its hash mod by a number of buckets.

    # Embed the instruction. Embedding size 20 seems to be enough.
    embedding_size = 20
    embedding = snt.Embed(num_hash_buckets, embedding_size)(buckets)# shape (None, None, 20)

    # Pad to make sure there is at least one output.
    padding = tf.to_int32(tf.equal(tf.shape(embedding)[1], 0))
    embedding = tf.pad(embedding, [[0, 0], [0, padding], [0, 0]])# ensure at least having shape (None, 1, 20)

    core = tf.contrib.rnn.LSTMBlockCell(64, name='language_lstm')
    output, _ = tf.nn.dynamic_rnn(core, embedding, length, dtype=tf.float32)# (cell, inputs, sequence_length, ...) returns output of shape (batch_size, max_time, 64), and state of shape (batch_size, 64)

    # Return last output.
    return tf.reverse_sequence(output, length, seq_axis=1)[:, 0]# shape (None, 64)
  ...
  def _torso(self, (last_action, env_output)):
    ...
    conv_out = tf.nn.relu(conv_out)# (unroll*batch_size, 256)

    instruction_out = self._instruction(instruction)# actually (unroll*batch_size, 64)

    # Append clipped last reward and one hot last action.
    clipped_reward = tf.expand_dims(tf.clip_by_value(reward, -1, 1), -1)# (unroll*batch_size, 1)
    one_hot_last_action = tf.one_hot(last_action, self._num_actions)# (unroll*batch_size, self._num_actions)
    return tf.concat(
        [conv_out, clipped_reward, one_hot_last_action, instruction_out],
        axis=1)# (unroll*batch_size, 256+1+9+64=330)

  def _head(self, core_output):# shape of core_output (None, 256) and (unroll*batch_size,  256) here
    policy_logits = snt.Linear(self._num_actions, name='policy_logits')(
        core_output)# (None, _num_actions)
    baseline = tf.squeeze(snt.Linear(1, name='baseline')(core_output), axis=-1)# (None)

    # Sample an action from the policy.
    new_action = tf.multinomial(policy_logits, num_samples=1,
                                output_dtype=tf.int32)
    new_action = tf.squeeze(new_action, 1, name='new_action')# (None)

    return AgentOutput(new_action, policy_logits, baseline)

  def _build(self, (action, env_output), core_state):# recurrent module inherits snt.RNNCore and in _build() method, builds a TF subgraph that performs one timestep of computation
    actions, env_outputs = nest.map_structure(lambda t: tf.expand_dims(t, 0),
                                              (action, env_output))# (func, *strucutre, **check_types_dict), applies func to each entry of `structure` and returns a new structure
    outputs, core_state = self.unroll(actions, env_outputs, core_state)
    return nest.map_structure(lambda t: tf.squeeze(t, 0), outputs), core_state# generally for recurrent module, return outputs, next_state

  @snt.reuse_variables# The most notable difference is that a single tf.VariableScope will be used across different decorated methods and each decorated method has its own reuse flag that is used to enter the variable scope. However every decorated method must be the sole owner of its variables.
  def unroll(self, actions, env_outputs, core_state):# actions=<shape=(unroll, batch_size) dtype=int32>, env_outputs=StepOutput(reward=<shape=(unroll, batch_size) dtype=float32>, info=StepOutputInfo(episode_return=<shape=(unroll, batch_size) dtype=float32>, episode_step=<shape=(unroll, batch_size) dtype=int32>), done=<shape=(unroll, batch_size) dtype=bool>, observation=[<shape=(unroll, batch_size, 72, 96, 3) dtype=uint8>, <shape=(unroll, batch_size) dtype=string>]) core_state=LSTMStateTuple(c=<shape=(batch_size, 256) dtype=float32>, h=<shape=(batch_size, 256) dtype=float32>)
    _, _, done, _ = env_outputs

    torso_outputs = snt.BatchApply(self._torso)((actions, env_outputs))# first reshape as (unroll*batch_size, ...) and apply self._torso, reshape back before return the final results, (unroll, batch_size, 330) here
    ...
    initial_core_state = self._core.zero_state(tf.shape(actions)[1], tf.float32)# (batch_size, 256)
    core_output_list = []
    for input_, d in zip(tf.unstack(torso_outputs), tf.unstack(done)):# unroll tuples each is (shape(batch_size, 330), shape(batch_size))
      # If the episode ended, the core state should be reset before the next.
      core_state = nest.map_structure(functools.partial(tf.where, d),# map_structure() see line198
                                      initial_core_state, core_state)# if done is True, use zero initial state. otherwise, use the state provided by actors
      core_output, core_state = self._core(input_, core_state)# __call__(inputs, state, ...) run this RNN cell on inputs, starting from the given state, inputs of shape (batch_size, input_size), state of shape (batch_size, state_size), returns output of shape (batch_size, output_size), state of the same shape as input state
      core_output_list.append(core_output)

    return snt.BatchApply(self._head)(tf.stack(core_output_list)), core_state# stacking results a shape (unroll, batch_size, 256), returns an AgentOutput


def build_actor(agent, env, level_name, action_set):
  """Builds the actor loop."""
  # Initial values.
  initial_env_output, initial_env_state = env.initial()# StepOutput(.0, (.0, .0), True, deepmind_lab.Lab.reset()), ('initial_flow'=0, (.0, 0)) where 'initial_flow' conditionally depends on the deepmind_lab.Lab.reset()
  initial_agent_state = agent.initial_state(1)# batch_size is 1
  initial_action = tf.zeros([1], dtype=tf.int32)# as a part of features to LSTM, batch_size is 1
  dummy_agent_output, _ = agent(# see the function signature of _build()
      (initial_action,
       nest.map_structure(lambda t: tf.expand_dims(t, 0), initial_env_output)),
      initial_agent_state)# dummy_agent_output is an AgentOutput(new_action, policy_logits, baseline) where 'T' is squeezed, i.e., shape of [1], [1, _num_actions], [1]
  ...
  def create_state(t):
    ...
    with tf.variable_scope(None, default_name='state'):
      return tf.get_local_variable(t.op.name, initializer=t, use_resource=True)# Behavior is the same as in get_variable, except that variables are added to the LOCAL_VARIABLES, when 'use_resource' is True, constructs a ResourceVariable
  ...
  with tf.control_dependencies(nest.flatten(assign_ops)):
    ...
    agent_outputs = nest.map_structure(lambda t: t[:, 0], agent_outputs)# [T], [T, _num_action], [T]
...
def create_environment(level_name, seed, is_test=False):
  ...
  p = py_process.PyProcess(environments.PyProcessDmLab, level_name, config,
                           FLAGS.num_action_repeats, seed)# __init__(type_, *constructor_args, **constructor_kwargs), proxy property returns a _TFProxy object that creates TF ops for each method call
  return environments.FlowEnvironment(p.proxy)# expose: initial() returns initial_output, initial_state and step(action, state) returns output, new_state


@contextlib.contextmanager
def pin_global_variables(device):# put all GLOBAL_VARIABLES on the given device, default 'collections' is None for tf.get_variable() method
...
def train(action_set, level_names):
  ...
  with tf.Graph().as_default(), \
    ...
    with tf.device(shared_job_device):
      queue = tf.FIFOQueue(1, dtypes, shapes, shared_name='buffer')# (capacity, dtypes, shapes=None, names=None, shared_name=None, name='fifo_queue')
      agent = Agent(len(action_set))# rnn parameters are defined in the constructor so the parameters are located on cpu
    ...
    if is_learner and enqueue_ops:
      tf.train.add_queue_runner(tf.train.QueueRunner(queue, enqueue_ops))# add the given queue runner to a given collection (default tf.GraphKeys.QUEUE_RUNNERS)    a QueueRunner holds a list of enqueue operations for a queue, each to be run in a thread
    ...
    if is_learner:
      ...
      dequeued = queue.dequeue_many(FLAGS.batch_size)# Dequeues and concatenates n elements from this queue, i.e., [component1, component2, ...] each component has 'n' as first dimension
      dequeued = nest.pack_sequence_as(structure, dequeued)# (structure, flat_sequence) returns a given flattened sequence packed into a given structure

      def make_time_major(s):
        return nest.map_structure(
            lambda t: tf.transpose(t, [1, 0] + range(t.shape.ndims)[2:]), s)

      dequeued = dequeued._replace(#replacing specified fields with new values
          env_outputs=make_time_major(dequeued.env_outputs),# old: (batch_size, unroll), new: (unroll, batch_size)
      ...
      with tf.device('/gpu'):
        ...
        # policy lag.
        flattened_output = nest.flatten(dequeued)# returns a (flattened) list of tensors
        area = tf.contrib.staging.StagingArea(
            [t.dtype for t in flattened_output],
            [t.shape for t in flattened_output])# default capacity=0 indicating unbounded
        stage_op = area.put(flattened_output)# this operation will block if the StagingArea has reached its capacity

        data_from_actors = nest.pack_sequence_as(structure, area.get())

        # Unroll agent on sequence, create losses and update ops.
        output = build_learner(agent, data_from_actors.agent_state,# LSTMStateTuple(c=<shape=(batch_size, 256) dtype=float32>, h=<shape=(batch_size, 256) dtype=float32>)
                               data_from_actors.env_outputs,# StepOutput(reward=<shape=(unroll, batch_size) dtype=float32>, info=StepOutputInfo(episode_return=<shape=(unroll, batch_size) dtype=float32>, episode_step=<shape=(unroll, batch_size) dtype=int32>), done=<shape=(unroll, batch_size) dtype=bool>, observation=[<shape=(unroll, batch_size, 72, 96, 3) dtype=uint8>, <shape=(unroll, batch_size) dtype=string>])
                               data_from_actors.agent_outputs)# AgentOutput(action=<shape=(unroll, batch_size) dtype=int32>, policy_logits=<shape=(unroll, batch_size, 9) dtype=float32>, baseline=<shape=(unroll, batch_size) dtype=float32>)
    ...
    with tf.train.MonitoredTrainingSession(
      ...
      if is_learner:
        ...
        session.run_step_fn(# warm up the stage area
...
def main(_):
  ...
  if FLAGS.level_name == 'dmlab30' and FLAGS.mode == 'train':
    ...
    level_names = [FLAGS.level_name]# default 'explore_goal_locations_small'
...
if __name__ == '__main__':
  tf.app.run()# tf.app.run(main=None, argv=None)
