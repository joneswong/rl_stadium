    ...
    with tf.train.MonitoredTrainingSession(
        ...
        if is_learner:
            ...
            while not is_end:
                ...
                episode_rwd += rwd
                is_end = info.get("is_end", False)
                if done:
                    cum_rewards.append(episode_rwd)
                    if len(cum_rewards) % 1024 == 0:
                        print("mean_cumulative_rewards={} over {} episodes".format(np.mean(cum_rewards), len(cum_rewards)))
                    episode_rwd = .0
                    obs = env.reset()
